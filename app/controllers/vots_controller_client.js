(function(){
		var ul = document.getElementById('voting');
		var url = '/vots';
		var id = 0;
		function update(data){
			if(data!==''){
				var vots = data.slice(1,data.length-1).replace(/\],\[/g,']||[').replace(/\[|\]/g,'');
				vots = vots.split('||');
				vots.forEach(function(ele,index,arr){
						vot = ele.split(',');
						var li = document.createElement('li');
						var a = document.createElement('a');
						a.setAttribute('href','#');
						//a.setAttribute('id',vot['_id']);
						id++;
						a.innerHTML = vot[1];
						li.appendChild(a);
						li.setAttribute('onclick','look('+vot[0]+')');
						ul.appendChild(li);
				});
				
			}else{
				var vot_option = document.getElementById('vot_option');
				var p = document.createElement('p');
				p.innerHTML = 'there seems NO voting,go to home to add some.'
				p.style['color']='red';
				vot_option.appendChild(p);
...
var deletMe = document.getElementById('deletMe');
...
var look = function(id){
	...
	function update(data){
		if(data){
			...
			deletMe.setAttribute('href','/delete/'+id);
